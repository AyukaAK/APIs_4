...
# API Key: 40f206f695b20cbf4a826a4141e32182
...
#Add "&limit=50" and "&artist=lil"
url = 'http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=40f206f695b20cbf4a826a4141e32182&format=json&limit=50'
...
# Explore the data
print(data['results']['artistmatches']['artist'][0].keys())
...
# for the number of listeners
print(data['results']['artistmatches']['artist'][0]['listeners'])            


# In[322]:


for lil in data['results']['artistmatches']['artist']:
    print(f"{lil['name']} has {lil['listeners']} listners")
...
listners = 0
for lil in data['results']['artistmatches']['artist']:   
    listners = listners + int(lil['listeners'])

print (listners)
print (f'total number of listeners is{listners: ,}')
...
# Explore the data
print(data['results']['artistmatches']['artist'][0]['image'][3]['#text'])
...
for lil in data['results']['artistmatches']['artist']:
    if lil ['name'] == 'Lil Jon':
        print(f"Lil Jon's mbid is {lil['mbid']}")
...
# remove "&artist=" and add "&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2" which is the mbid of Lil Jon
url_liljon = 'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2&api_key=40f206f695b20cbf4a826a4141e32182&format=json'
response_liljon = requests.get(url_liljon)
...
#Explore the data
print(data_liljon['artist'].keys())


# In[329]:


print("Here is the Bio for", data_liljon['artist']['name'], ":")
print(data_liljon['artist']['bio']['content'])
...
#Explore tags
print(data_liljon['artist']['tags']['tag'][0]['name'])
...
for tag in data_liljon['artist']['tags']['tag']:
    print (tag['name'])
...
artist_url = 'http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=40f206f695b20cbf4a826a4141e32182&format=json&limit=50'
...
#Explore the data 
print (artist_data['results']['artistmatches']['artist'][0]['mbid'])


# In[334]:


for artist in artist_data['results']['artistmatches']['artist']:
    if artist['mbid'] != "":
        print (artist['mbid'])
...
# url of each artist's API looks like this (from Lil Jon)  
#'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=40f206f695b20cbf4a826a4141e32182&format=json&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2'

for artist_API in mbids:
    print(f"http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=40f206f695b20cbf4a826a4141e32182&format=json&mbid={artist_API}")
...
for API_URL in mbids:
    Artist_API_List.append(f"http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=40f206f695b20cbf4a826a4141e32182&format=json&mbid={API_URL}")


# In[338]:


print (Artist_API_List)
...
#print(data_liljon['artist']['name'])


# In[342]:


Three_API_List = Artist_API_List [ : 3]


# In[343]:


for url_artist in Three_API_List:
...
for url_artist in Three_API_List:
    ...
    for tag_artist in data_artist['artist']['tags']['tag']:
        print (tag_artist['name'])
...
Ten_API_List = Artist_API_List [ : 10]
...
#Explore the idea
for url_artist in Ten_API_List:
    response_artist = requests.get(url_artist)
    data_artist = response_artist.json()
    print(data_artist['artist']['tags']['tag'][0]['name'])
...
for url_artist in Ten_API_List:
    response_artist = requests.get(url_artist)
    data_artist = response_artist.json()
    for artist in data_artist['artist']['tags']['tag']:
        if artist ['name'] == 'hip hop':
             hiphop = hiphop + 1
#hiphop artists
print (hiphop)

#total artists
len(data_artist['artist']


# In[ ]:


rappers = hiphop/len(data_artist['artist'])
print (round(rappers*100), "percent of lil results are rappers")
